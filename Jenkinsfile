...
pipeline {
    ...
    stages {
        stage('remove and clone') {
            steps {
                sh 'rm -rf gol'
                sh 'git clone https://github.com/Pathik120/gol'
            }
        }
            ...
            steps {
                script {
                    ...
                    if (!result.isEmpty()) {
                        ...
                        //sh 'docker rm jenkins_test
						//sh 'docker rmi pathik120/gol_image:prevBuild'
                    }
                }	
            }
        }
        stage('docker build and push') {
            steps 
            {
                sh 'docker build -t pathik120/gol_image:$BUILD_NUMBER ./gol/'
                //sh 'docker tag gol_image chiragvasani36/gol_image:$BUILD_NUMBER'
                script {
                    docker.withRegistry( '', registryCredential ) {
                    
                    sh 'docker push pathik120/gol_image:$BUILD_NUMBER' 
                    }
                }
            }
	    }
        ...
        stage('docker run') {
            steps {
                sh 'docker container run -d -it -p 8083:8080 --name jenkins_test -v /var/lib/tomcat8/conf/tomcat-users.xml:/usr/local/tomcat/conf/tomcat-users.xml pathik120/gol_image:$BUILD_NUMBER'
            }
        }
    }
